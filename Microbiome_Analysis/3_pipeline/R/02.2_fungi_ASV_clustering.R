...
#### CHANGE THIS PATHS ####
source <- "your_path_to_file_source"
taxa_database <- "your_path_to_database" #we used sh_general_release_dynamic_02.02.2019.fasta
##########################
...
if(output){
  ...
  pdf(paste(path_out,"fungi_reads_quality_unfilt_untrim.pdf",sep=""))
  ...
  pdf(paste(path_out,"fungi_reads_quality_filt_trim.pdf",sep=""))
  for (i in 1:length(sample.names)) {
  ...
  pdf(paste(path_out,"fungi_error_rate.pdf",sep=""))
  ...
  #rarefaction plots
  pdf(paste(path_out,"fungi_rarefaction.pdf",sep=""))
  for (run in runs) {
  ...
  #tables
  write.table(seqtab.nochim, paste(path_out, "fungi_ASV.tab", sep=""), sep="\t")
  if(taxonomy){write.table(taxa, paste(path_out, "fungi_taxa.tab", sep=""), sep="\t")}
  write.table(ASV_seq, paste(path_out, "fungi_sequences.fasta", sep=""), quote=F, row.names = FALSE, col.names = FALSE)
  write.xlsx(track, paste(path_out, "fungi_track.xlsx", sep=""))
}