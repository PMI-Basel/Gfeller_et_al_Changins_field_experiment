...
### Figure 9.1 | Bacteria: Correlation (microbiota) ~ PCA (soil chemistry) 
...
plot_cor <- function(df, cor, PCoA_axis, ylabel, pos_field, xlabel){
  plot <- ggplot(df, aes(y=get(PCoA_axis), x=get(pos_field), colour = Conditioning)) +
            geom_jitter(width = 0.1, size=3, aes(shape=Conditioning)) + 
            scale_colour_manual(values = level_cols_Conditioning) +
...
#combine plots
legend <- get_legend(p1 + theme(legend.position="bottom") + labs(shape="Conditioning", col="Conditioning"))
...
### Figure 9.2 | Fungi: Correlation (microbiota) ~ PCA (soil chemistry) 
...
```{r a-div maize plot, echo=F, message=F, warning=F, eval=T, fig.height=3, fig.width=8}
...
plotALPHA <- function(data, title="", label=""){
  p <- ggplot(data, aes(y=ASV_diversity_all, x=Sample_type, fill=Conditioning, col=PCA_soil_chem_1)) +
        ...
        theme_bw() +
        scale_fill_manual(values=level_cols_Conditioning) + #level_cols_Conditioning
...
*Conclusion:* The chemical field gradient did not influence the alpha diversity of the microbial communities in bare soil.
...
*Conclusion:* The chemical gradient affected bacterial but not fungal communities in bare-soil. No difference in beta diversity were found between communities from different conditioned soils.
